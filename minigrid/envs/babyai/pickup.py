...
class PickupLoc2(LevelGen):
    ...
    This environment is similar to `PickupLoc`, but with the following differences:
    - There is always a location in the instruction.

    Competencies: PickUp, Loc. No unblocking.

    ## Mission Space

    "pick up the {color} {type} {location}"
    ...
    {location} is the location of the object. Can be "front", "behind", "left" or "right".
    ...
    def gen_mission(self):
        ...
        while self.grid.get(*obj_coords) != None :
            ...
            counter -= 1
            if counter == 0:
                raise RejectSampling("could not place object")
    ...
    def get_obj_coords(self, loc, agent_pos):
        if loc == "left":
            # Then the object should be on the right
            if self.dir_vec[0] > 0:
                # This means that the agent is facing right
                x_range = (1, self.room_size)
                y_range = (agent_pos[1], self.room_size)
            elif self.dir_vec[0] < 0:
                # This means that the agent is facing left
                x_range = (1, self.room_size)
                y_range = (1, agent_pos[1])
            elif self.dir_vec[1] > 0:
                # This means that the agent is facing down
                x_range = (1, agent_pos[0] + 1)
                y_range = (1, self.room_size)
            elif self.dir_vec[1] < 0:
                # This means that the agent is facing up
                x_range = (agent_pos[0], self.room_size)
                y_range = (1, self.room_size)
      
        elif loc == "right":
            # Then the object should be on the left
            if self.dir_vec[0] > 0:
                x_range = (1, self.room_size)
                y_range = (1, agent_pos[1])
            elif self.dir_vec[0] < 0:
                ...
                x_range = (1, agent_pos[0])
                y_range = (1, self.room_size)
          
        elif loc == "front":
            # Then the object should be behind
            if self.dir_vec[0] > 0:
                x_range = (1, agent_pos[0])
                y_range = (1, self.room_size)
            elif self.dir_vec[0] < 0:
            ...
            # Then the object should be in front
            if self.dir_vec[0] > 0:
                x_range = (agent_pos[0], self.room_size)
                y_range = (1, self.room_size)
            elif self.dir_vec[0] < 0:
        ...
        # Check that there is any space left to place the object
        if x_range[0] == x_range[1] or y_range[0] == y_range[1]:
            raise RejectSampling("no space to place object")
        
        # Randomly sample a location for the object, from the valid range
        obj_x = self._rand_int(*x_range)
        obj_y = self._rand_int(*y_range)
        
        counter = 10
        while obj_x == agent_pos[0] and obj_y == agent_pos[1]:
            obj_x = self._rand_int(*x_range)
            obj_y = self._rand_int(*y_range)
            counter -= 1
            if counter == 0:
